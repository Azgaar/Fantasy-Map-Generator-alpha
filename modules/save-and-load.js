...
function saveMap() {
  ...
  const gridGeneral = JSON.stringify({spacing:grid.spacing, cellsX:grid.cellsX, cellsY:grid.cellsY, boundary:grid.boundary, points:grid.points, features:grid.features}, null, 2);
  const features = JSON.stringify(pack.features, null, 2);
  const cultures = JSON.stringify(pack.cultures, null, 2);
  const states = JSON.stringify(pack.states, null, 2);
  const burgs = JSON.stringify(pack.burgs, null, 2);
